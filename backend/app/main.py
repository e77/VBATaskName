...
def _hydrate_slot(slot: Slot) -> Slot:
    spool = SPOOLS.get(slot.spool_id) if slot.spool_id else None
    slot.spool = spool if isinstance(spool, Spool) else None
    return slot
...
def _serialize_unit(unit: AmsUnit) -> Dict[str, Any]:
    ...
    for slot in sorted(unit.slots, key=lambda s: s.slot_number):
        hydrated = _hydrate_slot(slot).dict()
        spool = SPOOLS.get(slot.spool_id)
        hydrated["spool"] = spool.dict() if isinstance(spool, Spool) else None
